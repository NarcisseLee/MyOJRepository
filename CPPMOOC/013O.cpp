// Other person's code, only for the reference
// source from website:https://www.cnblogs.com/vv123/p/16031397.html
// copyright by vv123
#include <iostream>
#include <string>
...
int main() {
  ...
  for (int k = 1; k <= T; k++) {
    ...
    for (int time = 0; !red.runout  || !blue.runout; time++) {
      if (!red.runout) red.work(time);
      if (!blue.runout) blue.work(time);
    }
  }
  return 0;
}
